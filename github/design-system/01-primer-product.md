# 01 · Primer Product UI · Full Spec

> Sections 1-9 from the legacy single-file `design-system.md`.
> System: **Primer 产品 UI**（适用于 logged-in 工具型界面）
> Token prefix: `--gh-product-*` · Font: system stack · Preview: `../previews/primer-product-light.html` / `-dark.html`
>
> 跨系统决策：先读 `00-overview.md`。开工硬规则：读 `03-hard-rules.md`。
